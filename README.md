...
Halo 2 は Electric Coin Company（Zcash の開発者）によって、Zcash の非効率性やセキュリティ上の問題（トラステッド・セットアップなど）を解決するために開発された。Halo 2 の証明システムは、信頼されたセットアップなしで zkSNARK 証明の作成と検証を可能にする一連の Rust クレートによって実装されているライブラリ。

回路、証明、証明/検証キーなどを生成するために作成する関数は、すべて WebAssembly にコンパイルすることができる。

## Chips とは

前節では、回路をかなり低レベルで記述した。回路を実装する場合、私たちは通常、監査性、効率性、モジュール性、表現力といった望ましい特性を目指した、より高レベルの API を使用する。

この API で使用される用語と概念の一部は、集積回路の設計とレイアウトとの類似から取られている。集積回路と同様に、上記の望ましい特性は、特定の機能の効率的な組み込み済み実装を提供するチップを構成することで得やすくなる。

例えば、ハッシュ関数や暗号のような特定の暗号プリミティブや、スカラー乗算やペアリングのようなアルゴリズムを実装するチップがあるかもしれない。

PLONKish 回路では、フィールド乗算や加算を行う標準的なゲートだけで任意の論理を構築することが可能です。しかし、カスタムゲートを使うことで、非常に大きな効率向上が得られる。

私たちの API を使うことで、カスタムゲートの特定のセットの使い方を「知っている」チップを定義することができます。これにより、高レベル回路の実装を、カスタム・ゲートを直接使用する複雑さから切り離す抽象化レイヤーを作成します。
...
R1CS（読者によっては馴染みがあるかもしれないが、そうでなくても気にしないでほしい）では、回路は意味的に重要な順序を持たない「ゲートの海」で構成される。一方、PLONK 的回路では、オフセット参照のため、行の順序は重要である。
...
DeFi のサーキットブレーカーは重要な安全メカニズムです。DeFi のエコシステムにさらなる安全性と安定性をもたらすのに役立ちます。サーキットブレーカーとは、急激な価格下落や未検出のセキュリティ脆弱性など、不測の事態や異常事態が発生した場合に、プロトコルの特定の活動を停止または一時停止するように設計されたメカニズムです。

サーキット・ブレーカーの役割
サーキット・ブレーカーは、次のような多くの潜在的なセキュリティ問題の程度を軽減するのに役立ちます：

突然の価格下落-価格が大幅に暴落すると、プロトコルがセキュリティ攻撃ベクトルや操作に対して脆弱な状態になる可能性がある。
大幅な流動性の除去-突然大量の流動性が除去されると、正常な運用のために最適な流動性条件に依存しているプロトコルでカスケード障害が発生する可能性がある。
端数準備活動/担保不足-十分に担保されていない準備は、プロトコルの失敗や担保不足の資産につながる可能性がある。
価格異常-異常な価格変動はプロトコルの運用を混乱させ、攻撃、ハッキング、悪用にさらされる可能性がある。
ステーブル/ペッグ資産のデペッグ-ステーブルコインとペッグ資産は、ターゲット価格またはインデックスとのパリティを維持することを目的としています。ペッグが崩れると、資産を利用するプロトコルに連鎖的な障害が発生する可能性がある。
セキュリティ上の問題が検出された場合、サーキットブレーカーが起動し、問題が評価され解決されるまで活動を停止または一時停止し、インシデントの結果としてのプロトコルやユーザーの損失を防ぐのに役立つ。サーキット・ブレーカーは、1 つのセキュリティ脆弱性が分散型アプリケーションの相互接続ループに有害なドミノ効果を引き起こす、DeFi におけるカスケード障害の防止にも役立ちます。
...
57. [Halo2 で使われている最新の zk-SNARK プロトコル PLONK の数学的解説](https://tech.hashport.io/3711/)
...
63. [DeFi Circuit Breakers With Chainlink Proof of Reserve and Automation](https://blog.chain.link/defi-circuit-breakers/)
64. [What is ERC7265: Enhancing DeFi Security with a Circuit Breaker](https://blog.bunzz.dev/what-is-erc7265/)
65. [DeFi ハッキング対策、イーサリアム改善案「ERC 7265」とは？](https://coinpost.jp/?p=471084)
...
73. [Aelo 公式サイト](https://aleo.org/)
74. [Celo - Hardhat 設定ファイル](https://docs.celo.org/developer/deploy/hardhat)
75. [Celo - Faucet ページ](https://faucet.celo.org/alfajores)
76. [Celo - 開発者ページ](https://docs.celo.org/)
77. [Celo - Explorer](https://explorer.celo.org/alfajores/)
78. [Aelo Docs - Getting started](https://developer.aleo.org/getting_started/)
79. [Aelo - Faucet Page](https://faucet.aleo.org/)
80. [Aelo - Leo Example](https://developer.aleo.org/leo/examples/)
...
86. [Green Zkrciticty - SourceCode](https://github.com/DappaDanDev/green-zktricity_)
87. [AirdropFi](https://ethglobal.com/showcase/airdropfi-6f65w)
88. [AztecSnap](https://ethglobal.com/showcase/aztecsnap-prn4s)
89. [merhaba-wallet](https://ethglobal.com/showcase/merhaba-wallet-5b2ke)
90. [Celo AA Wallet](https://github.com/celo-academy/erc-4337-dApp/commit/300a640091e566ef97195aa7ccca561da016c35c)
91. [Trippy - Access ZK Verified event / storage information on any EVM L2](https://ethglobal.com/showcase/trippy-kzvih)
92. [Themis: ZKML Loyalty](https://ethglobal.com/showcase/themis-zkml-loyalty-azin8)
...
99. [AZTEC Protocol とは？Ethereum の取引を秘匿化](https://baasinfo.net/?p=4138)
100. [Aztec（アステカ）とは？プライバシー問題を解決するレイヤー 2 の評判・将来性・特徴についてわかりやすく解説！](https://tottemoyasashiibitcoin.net/entry/2022/08/01/1611#google_vignette)
101. [イーサリアム L2「Aztec」、新たなプライバシーソリューションをローンチ](https://coinpost.jp/?p=357827)
...
106. [sindri 公式サイト](https://sindri.app/)
107. [Zero-knowledge プラットフォームの Sindri が CoinFund 主導の 500 万ドルのシードラウンドを獲得](https://crypto-ai-digest.com/2023/12/07/1701954842733/)
108. [Sindri ブログ](https://sindri.app/blog/)
...
112. [Noir Contract 実装例](https://github.com/AztecProtocol/aztec-packages/tree/aztec-packages-v0.17.0/yarn-project/noir-contracts/contracts)
...
115. [RPC 情報](https://docs.availproject.org/networks/)
116. [Avail 開発者ドキュメント](https://docs.availproject.org/)
