...
impl<F: Field> Circuit<F> for HammsterCircuit<F> {
  ...
  fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
    ...
    // HammsterChipのconfigureメソッドを呼び出す。
...
pub fn draw_circuit<F: Field>(
  k: u32, 
  circuit: &HammsterCircuit<F>
) {
...
pub fn generate_keys(
  ...
  // just to emphasize that for verify key, proving key we don't need to know the value of `x`
