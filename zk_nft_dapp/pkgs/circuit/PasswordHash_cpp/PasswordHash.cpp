...
void SecretHasher_11_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void SecretHasher_11_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 7878;}

uint get_number_of_components() {return 749;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 687;}
...
ctx->componentMemory[coffset].templateName = "SecretHasher";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint aux_create = 0;
int aux_cmp_num = 1+ctx_index+1;
uint csoffset = mySignalStart+251;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "secretHasher";
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[17]); // line circom 18
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
SecretHasher_11_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 7875 ;
aux_cmp_num += 748;
}
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
SecretHasher_11_run(mySubcomponents[cmp_index_ref],ctx);
}
Fr_eq(&expaux[0],&signalValues[mySignalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]); // line circom 37
